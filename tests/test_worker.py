...
def generate_test_docker_worker_config(
    home_directory: Path, name: str = "test_docker_worker"
) -> WorkerConfig:
    """Generate ``WorkerConfig`` for use in Docker-backend test cases."""
    return WorkerConfig(
        name=name,
        server_address=f"http://localhost:{TEST_SERVER_PORT}",
        home_directory=home_directory,
...
@pytest.mark.asyncio
async def test_register_with_docker_backend() -> None:
    """Test worker registration with Docker backend."""
    with TemporaryDirectory(delete=True) as data_dir:
        configure_server_for_test(Path(data_dir))
        ...
        async with (
            ...
            registration_data = WorkerRegistration(
                name="docker_worker", backends={WorkerBackend.DOCKER}
            )
            worker = await Worker.register(
                client=client, registration_data=registration_data
            )
            assert isinstance(worker, WorkerData)
            assert worker.registration_data.name == "docker_worker"
            assert WorkerBackend.DOCKER in worker.registration_data.backends


@pytest.mark.asyncio
async def test_get_work_returns_none_when_idle_docker(tmp_path: Path) -> None:
    """get_work returns None when the server has no work (Docker worker)."""
    configure_server_for_test(tmp_path / "server")
    ...
    async with (
        ...
        metadata = await Worker.register(
            client=client,
            registration_data=WorkerRegistration(
                name="docker_idle_worker", backends={WorkerBackend.DOCKER}
            ),
        )
        worker = Worker(
            http_client=client,
            meta_data=metadata,
            config=generate_test_docker_worker_config(home_directory=home_dir),
        )
...
@pytest.mark.asyncio
async def test_get_work_with_docker_backend(
    tmp_path: Path,
    make_experiment: Callable[..., tuple[ExperimentDefinition, Path]],
) -> None:
    """Worker with Docker backend can fetch dispatched work from the server."""
    configure_server_for_test(tmp_path / "server")
    ...
    async with (
        ...
        home_dir = tmp_path / "worker"
        registration_data = WorkerRegistration(
            name="docker_w", backends={WorkerBackend.DOCKER}
        )
        metadata = await Worker.register(
            client=client, registration_data=registration_data
        )
        ...
        definition, experiment_dir = make_experiment(backends={WorkerBackend.DOCKER})
        (experiment_dir / "Dockerfile").write_text(DOCKERFILE_STUB)
...
@pytest.mark.asyncio
async def test_report_error_with_docker_backend(
    tmp_path: Path,
    make_experiment: Callable[..., tuple[ExperimentDefinition, Path]],
) -> None:
    """Worker with Docker backend reports an error; server records the failure."""
    configure_server_for_test(tmp_path / "server")
    ...
    async with (
        ...
        failure = RunFailureError(run=run, reason=Exception("DOCKER_ERROR"))
        ...
        assert submitted_run.active_state == ActiveState.FINISHED
        assert submitted_run.success_state == SuccessState.FAILED
        assert submitted_run.failure_reason == "DOCKER_ERROR"
