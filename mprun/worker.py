...
class Worker:
    ...
    Attributes:
        ...
        working (Run | None): Run-object if we are currently executing a run.
                              None otherwise.
        home_dir (Path): Worker home directory for storing Experiment archives & other data.
        execution_dir (Path): Will be used as working directory for Run execution.
        archive_path (Path): Path to store archive of current Experiment (if one exists).
    """
    ...
    async def get_work(self) -> Run | None:
        ...
        Streams the Experiment archive from the server into a temporary directory and
        validates it against the Run's ExperimentDefinition. The validated archive is
        ...
        Raises:
            HTTPStatusError: If communication with server fails.
            ArchiveValidationError: If the received archive does not match the Run's ExperimentDefinition.
    ...
    async def collect_results(self) -> None:
        """Collect results from Experiment execution."""
        if self.working is None:
            raise NoRunError
